# Changelog

Tudo que mudarmos no projeto será documentado nesse arquivo.

## [Unreleased]
### Planejado
- Término da leitura dos materiais (parte 3).
- Estudar como obter os dados de maneira consistente.
- Tentativa inicial de obtenção dos dados.

---

## [0.2.0] - 2025-12-13
### Adicionado
- **Módulo 5:** Criação do notebook conceitual `notebook_mod5.ipynb`.
- Implementação de visualizações matemáticas em Python (Gráfico 3D da Maldição da Dimensionalidade e Swiss Roll).
- Textos explicativos sobre Feature Engineering, PCA e Autoencoders.

## [0.1.3] - 2025-12-12
### Atualizado
- Leitura da apostila.
- Criação inicial da estrutura do capítulo novo.

## [0.1.2] - 2025-12-06
### Atualizado
- Leitura dos materiais.
- Discussão do tema a ser trabalhado: definimos que será com dados do SUS (biblioteca PySUS para obtenção dos microdados públicos).
- Delimitação dos limites do nosso projeto.
- Reunião com tutores para domingo (12-06).

## [0.1.1] - 2025-11-28
### Atualizado
- Continuação da leitura e análise dos materiais de estudo (nenhuma alteração de código nesta semana).

## [0.1.0] - 2025-11-21
### Adicionado
- Criação do repositório.